...
////////////////////////////////////////////////////////////
#define DEWV_WATCHFILE_MAX  (DEWV_CONF_SOCKBUFF_MAX/MD5_DIGEST_LENGTH)
#define DEWV_INOTIFY_SIZE  (sizeof(struct inotify_event))
#define DEWV_INOTIFY_BUFFLEN    (DEWV_WATCHFILE_MAX*(DEWV_INOTIFY_SIZE+16))
...
static const char path_to_watch[] = DEWV_CONF_SERVER_SAMPLEDIR; 
...
int main(int argc, char *argv[])
{
    ...
    send_buff = malloc(DEWV_CONF_SOCKBUFF_MAX);
    if (NULL != send_buff) {
        /* Comment #1 */
        ...
        if (0 <= make_sendbuff(path_to_watch)) {
            ...
            if (0 >= pthread_create(&handler_thread, NULL,  sample_monitor, 0)) {
                ...
                perror("[dewv] Failed to pthread_create() to monitor");
            }
        } else {
            pthread_mutex_unlock(&send_lock);    
            perror("[dewv] Failed to make buffer to send");
        }
    } else {
...
int update_loop(void)
{
    ...
    if (sock_desc == -1) {
        perror("[dewv] Failed to create socket!");
        goto main_out;
    }
    ...
    if (bind(sock_desc,(struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        perror("[dewv] Failed to bind socket!");
        goto main_out;
    }
     
    listen(sock_desc, DEWV_CONF_CLIENT_MAX);
    printf("[dewv] Waiting for client ...\n");
    ret = 0;
    while ((cli_sock = accept(sock_desc, (struct sockaddr *)&cli_addr, (socklen_t*)&sock_len))) {
        printf("[dewv][%d client] Accepted\n", cli_sock);
        /* Comment #4 */        
        if (pthread_create( &handler_thread, NULL,  client_handler, (void*)(intptr_t)cli_sock) < 0) {
            perror("[dewv] Failed to create thread!");
            continue;
        }
    }
...
void * client_handler(void *sock_desc)
{
    ...
    int read_size   = 0;
    char cli_msg[DEWV_CONF_SOCKBUFF_MAX]   = {0x00, };
 
    read_size = recv(sock, cli_msg, DEWV_CONF_SOCKBUFF_MAX-1, 0);
    if (0 < read_size) {
        cli_msg[DEWV_CONF_SOCKBUFF_MAX-1] = 0;
        printf("[dewv][%d client] Received : \"%s\"\n", sock, cli_msg);
        do {
            pthread_mutex_lock(&send_lock);
            if(0 < send(sock, send_buff, send_buff_len, 0)) {
                printf("[dewv][%d client] Sent signatures\n", sock);
            } else {
                pthread_mutex_unlock(&send_lock);
                perror("[dewv] Failed to send!");
                break;
            }
        ...
        printf("[dewv][%d client] Disconnected\n", sock);
    } else {
        perror("[dewv] Failed to recevie");
    }        

    close(sock);
    printf("[dewv][%d client] Exit\n", sock);    
    return 0;
}
    ...
    if (dir != NULL) {
        ...
        while (!readdir_r( dir, &entry, &result ) && ( result != NULL )) {
            if (DT_REG == entry.d_type) {
                ++ret;
                if (send_buff_len + MD5_DIGEST_LENGTH + 2 < DEWV_CONF_SOCKBUFF_MAX) {
                    file_path[file_start] = 0;
                    strncat(file_path+file_start, entry.d_name, strlen(entry.d_name));
                    if (0 == dew_hash_get(file_path, 0, md5)) {
                    ...
                    printf("[%s] The count of sample files exceed DEWV_WATCHFILE_MAX(%d)\n",
                            __FUNCTION__, DEWV_WATCHFILE_MAX);
                    break;
                }
            }
        }
...
{    
    ...
    int dir_changed = 0;
    int fd, wd = 0;
    char buffer[DEWV_INOTIFY_BUFFLEN];
    struct inotify_event *event = NULL;
    
    printf("[dewv] monitor thread start\n");

    fd = inotify_init();
    if (fd < 0) {
        perror("[dewv] Failed to inotify_init()");
    }
    ...
    do
    {
        ...
        read_len = read(fd, buffer, DEWV_INOTIFY_BUFFLEN); 
        if (read_len < 0) {
            perror( "[dewv] Failed to read()" );
        }
        ...
        while (event_index < read_len) {
            ...
            if (event->len) {
                if (event->mask & IN_CLOSE_WRITE ||
                    ...
                    printf("[dewv] watchor detected some changes.\n");
                    dir_changed = 1;
                }
                event_index += DEWV_INOTIFY_SIZE + event->len;
            } else {
                event_index += DEWV_INOTIFY_SIZE;                
            }
        }
